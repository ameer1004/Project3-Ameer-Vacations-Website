...
interface UpdateVacationProps {
  vacation: any; // TODO create vactation object (export interface Vacation)
  setupdate: CallableFunction;
}

const UpdateVacation: FC<UpdateVacationProps> = ({ vacation, setupdate }) => {
  ...
  const updateVac = async () => {
    const resU = await fetch(
      `http://localhost:4000/adminpage/update/${vacation.vacationID}`,
      {
        method: "put",
  ...
  return (
    <div>
      ...
      >
        ...
        <Button variant="contained" onClick={updateVac}>
          Update
        </Button>
        <p>{msg}</p>
      </Box>
